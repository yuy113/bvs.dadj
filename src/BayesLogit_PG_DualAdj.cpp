...
static inline double calc_loglik(const arma::vec& y, const arma::vec& z,
    ...
    for (arma::uword i = 0; i < n; ++i) {
        double x = alpha + z(i);
        term1 += y(i) * x;
        if (x > 0.0)  term2 += x + std::log1p(std::exp(-x));
        else           term2 += std::log1p(std::exp(x));
    }
    return term1 - term2;
}
...
// =============================================================================
static Rcpp::IntegerVector proppwilson_omega(
    const Rcpp::IntegerMatrix& R1,
    ...
    unsigned int T_max)
{
    ...
    while (Rcpp::sum(x_up != x_down) > 0) {
        ...
        if (T >= T_max) {
            ...
            for (unsigned int k = 0; k < p; ++k) {
                if (x_up[k] != x_down[k]) diff_idx.push_back(k);
            }
            ...
            // Force coalescence
            for (unsigned int k = 0; k < p; ++k) x_down[k] = x_up[k];
        }
    }
...
// =============================================================================
static void moller_update_eta(
    const Rcpp::IntegerMatrix& R1,
    const Rcpp::IntegerMatrix& R2,
    double mu,
    double& eta1, double& eta2,
    double eta1_sd, double eta2_sd,
    ...
    int proposal_type)
{
    ...
    if (proposal_type == 0) {
        ...
        do {
            ...
            if (++attempts > 10000) { eta1_new = eta1; break; }
        } while (eta1_new <= 0.0 || eta1_new >= eta1_sd);

        attempts = 0;
        do {
            eta2_new = R::rnorm(eta2, eta2_sd);
            if (++attempts > 10000) { eta2_new = eta2; break; }
        ...
        double log_q_fwd_1 = std::log(normal_pdf(eta1_new, eta1, eta1_sd))
            - std::log(normal_cdf(eta1_sd, eta1, eta1_sd) - normal_cdf(0.0, eta1, eta1_sd));
        double log_q_rev_1 = std::log(normal_pdf(eta1, eta1_new, eta1_sd))
            - std::log(normal_cdf(eta1_sd, eta1_new, eta1_sd) - normal_cdf(0.0, eta1_new, eta1_sd));
        log_prop_ratio_eta1 = log_q_rev_1 - log_q_fwd_1;

        double log_q_fwd_2 = std::log(normal_pdf(eta2_new, eta2, eta2_sd))
            - std::log(normal_cdf(eta2_sd, eta2, eta2_sd) - normal_cdf(0.0, eta2, eta2_sd));
        double log_q_rev_2 = std::log(normal_pdf(eta2, eta2_new, eta2_sd))
            - std::log(normal_cdf(eta2_sd, eta2_new, eta2_sd) - normal_cdf(0.0, eta2_new, eta2_sd));
    ...
    Rcpp::IntegerVector omega_eta1     = proppwilson_omega(R1, R2, mu, eta1,     eta2,     T_max);
    Rcpp::IntegerVector omega_eta2     = proppwilson_omega(R1, R2, mu, eta1,     eta2,     T_max);
    // omega for proposed eta values
    Rcpp::IntegerVector omega_eta1_new = proppwilson_omega(R1, R2, mu, eta1_new, eta2,     T_max);
    Rcpp::IntegerVector omega_eta2_new = proppwilson_omega(R1, R2, mu, eta1,     eta2_new, T_max);
    ...
    // --- Prior ratio: log Beta(eta_new/eta_sd | e,f) - log Beta(eta/eta_sd | e,f) ---
    ...
    double log_norm_eta1 = mu * (sum_om1 - sum_om1n)
        + eta1 * A_om1_R1 - eta1_new * A_om1n_R1
        + eta2 * (A_om1_R2 - A_om1n_R2);

    double log_MH_eta1 = log_target_eta1 + log_aux_eta1
                        + log_norm_eta1 + log_prop_ratio_eta1;
    ...
    double log_norm_eta2 = mu * (sum_om2 - sum_om2n)
        + eta2 * A_om2_R2 - eta2_new * A_om2n_R2
        + eta1 * (A_om2_R1 - A_om2n_R1);

    double log_MH_eta2 = log_target_eta2 + log_aux_eta2
                        + log_norm_eta2 + log_prop_ratio_eta2;
...
// [[Rcpp::export]]
Rcpp::IntegerMatrix phase_transit_2eta_fixadj(
    Rcpp::IntegerMatrix R1, Rcpp::IntegerMatrix R2,
    int T_max, double mu,
    double min_eta, double max_eta,
    unsigned int num_rep, double step_size = 0.01)
{
    if (min_eta > max_eta) Rcpp::stop("min_eta must be <= max_eta");
    unsigned int p = R1.ncol();
    unsigned int len_eta = static_cast<unsigned int>((max_eta - min_eta) / step_size) + 1;
    ...
    for (unsigned int i = 0; i < len_eta; ++i) {
        for (unsigned int j = 0; j < num_rep; ++j) {
            Rcpp::IntegerVector g = proppwilson_omega(R1, R2, mu, eta_tmp, eta_tmp, T_max);
            int s = 0;
            for (unsigned int k = 0; k < p; ++k) s += g[k];
            gamma_output(i, j) = s;
        }
        eta_tmp += step_size;
    }
...
Rcpp::List BayesLogit_PG_DualAdj(
    ...
    Rcpp::IntegerMatrix R_glasso_int,
    Rcpp::IntegerMatrix R_fix_int,
    ...
    double     alpha_in)
{
    ...
    if ((unsigned int)R_glasso_int.nrow() != p || (unsigned int)R_glasso_int.ncol() != p)
        Rcpp::stop("R_glasso dimensions must match p = %d", p);
    if ((unsigned int)R_fix_int.nrow() != p || (unsigned int)R_fix_int.ncol() != p)
    ...
    double sigmasq = 1.0;
    double eta1 = 0.01;   // initial eta for R_glasso
    double eta2 = 0.01;   // initial eta for R_fix
    ...
    for (int iter = 0; iter < total_iter; ++iter) {
        ...
        if (iter % 10000 == 0 && iter > 0) {
            ...
            Rcpp::Rcout << "Iter: " << iter << " | Model size: "
                        << model_size
                        << " | eta1: " << eta1
                        << " | eta2: " << eta2 << "\n";
        }
        ...
        if (p_active > 0) {
            ...
            if (chol_ok) {
                arma::vec m_beta = arma::solve(arma::trimatl(L_prec.t()),
                                               mean_rhs, arma::solve_opts::fast);
                m_beta = arma::solve(arma::trimatu(L_prec), m_beta,
                                     arma::solve_opts::fast);
                arma::vec zz = arma::randn<arma::vec>(p_active);
                arma::vec b_draw = m_beta +
                    arma::solve(arma::trimatu(L_prec), zz,
                                arma::solve_opts::fast);
                beta.zeros();
        ...
        // --- Intercept alpha ---
        {
            ...
            double mean_alpha = var_alpha *
                (arma::accu(resid) + alpha0 / (h * sigmasq));
        ...
        for (int mh = 0; mh < n_mh_gamma; ++mh) {
            ...
            double b_prop = (g_prop == 1) ?
                R::rnorm(beta0, std::sqrt(sigmasq)) : 0.0;
            ...
            for (arma::uword i = 0; i < p; ++i) {
                if ((arma::uword)j == i) continue;
                if (R_glasso(i, j) != 0.0) neigh_glasso += gamma(i);
                if (R_fix(i, j)    != 0.0) neigh_fix    += gamma(i);
            }

            double ising_diff = diff * (mu + eta1 * neigh_glasso
                                            + eta2 * neigh_fix);

            // When proposal q(beta_j) = prior p(beta_j | gamma_j=1) = N(beta0, sigmasq),
            // the prior and proposal terms cancel exactly in the RJ-MCMC ratio.
            // No log_proposal_ratio correction is needed.
        ...
        // ==================================================================
        {
            ...
            double lp_sig_curr = -(shape + 1.0) * std::log(sigmasq)
                                 - scale / sigmasq;
            double lp_sig_prop = -(shape + 1.0) * std::log(sig_prop)
                                 - scale / sig_prop;
            ...
            double lp_b_c = -0.5 * n_act * std::log(sigmasq)
                            - 0.5 * ss_b / sigmasq;
            double lp_b_p = -0.5 * n_act * std::log(sig_prop)
                            - 0.5 * ss_b / sig_prop;

            double lp_a_c = -0.5 * std::log(h * sigmasq)
                            - 0.5 * (alpha - alpha0) * (alpha - alpha0)
                              / (h * sigmasq);
            double lp_a_p = -0.5 * std::log(h * sig_prop)
                            - 0.5 * (alpha - alpha0) * (alpha - alpha0)
                              / (h * sig_prop);
        ...
        // ==================================================================
        moller_update_eta(
            R_glasso_int, R_fix_int,
            mu, eta1, eta2,
            eta1_sd, eta2_sd,
            mu_tilde, eta1_tilde, eta2_tilde,
            gamma, e, f,
            T_max, proposal_type
        );
