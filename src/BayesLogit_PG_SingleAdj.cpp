...
// =============================================================================
static inline double calc_loglik(const arma::vec& y01, const arma::vec& z, double alpha) {
...
static void build_neighbors_edges(
...
// =============================================================================
static Rcpp::IntegerVector proppwilson_omega_1eta(
...
// =============================================================================
static void moller_update_eta_1(
    const NeighborList& neighbors,
    const EdgeList& edges,
    double mu,
    double& eta,
    double eta_sd,
    double mu_tilde, double eta_tilde,
    ...
    int proposal_type)
{
    ...
    if (proposal_type == 0) {
        ...
        do {
            ...
            if (++attempts > 10000) { eta_new = eta; break; }
        } while (eta_new <= 0.0 || eta_new >= eta_sd);

        double denom_fwd = std::max(1e-16, normal_cdf(eta_sd, eta, eta_sd) - normal_cdf(0.0, eta, eta_sd));
        double denom_rev = std::max(1e-16, normal_cdf(eta_sd, eta_new, eta_sd) - normal_cdf(0.0, eta_new, eta_sd));

        double log_q_fwd = std::log(normal_pdf(eta_new, eta, eta_sd)) - std::log(denom_fwd);
        double log_q_rev = std::log(normal_pdf(eta, eta_new, eta_sd)) - std::log(denom_rev);
    ...
    Rcpp::IntegerVector omega_cur = proppwilson_omega_1eta(neighbors, mu, eta,     T_max);
    Rcpp::IntegerVector omega_new = proppwilson_omega_1eta(neighbors, mu, eta_new, T_max);
    ...
    double log_prior = log_beta_pdf(eta_new / eta_sd, e, f)
                     - log_beta_pdf(eta     / eta_sd, e, f);
    ...
    double log_norm   = mu * (double)(sum_om - sum_om_new)
                      + eta * (double)A_om_R - eta_new * (double)A_om_new_R;

    double log_MH = log_target + log_aux + log_norm + log_prop_ratio;

    if (std::log(R::runif(0.0, 1.0)) < log_MH) eta = eta_new;
}
    ...
    double     sigmasq_init = 1.0)
{
    ...
    if ((unsigned int)R_adj_int.nrow() != p || (unsigned int)R_adj_int.ncol() != p)
    ...
    for (arma::uword i = 0; i < y01.n_elem; ++i) {
        double yi = y01(i);
        if (std::fabs(yi - 0.0) > 1e-12 && std::fabs(yi - 1.0) > 1e-12) is01 = false;
        if (std::fabs(yi + 1.0) > 1e-12 && std::fabs(yi - 1.0) > 1e-12) is11 = false;
    }
    if (!is01 && is11) {
        y01 = 0.5 * (y01 + 1.0);
    ...
    if (gamma.n_elem != p || beta.n_elem != p)
        Rcpp::stop("beta_init and gamma_init must have length p.");
    ...
    arma::vec  eta_out(niter), alpha_out(niter), sigmasq_out(niter);
    ...
    for (int iter = 0; iter < total_iter; ++iter) {
        ...
        if (p_active > 0) {
            ...
            arma::vec mean_rhs = X_act.t() * z_star + (beta0 / sigmasq) * arma::ones<arma::vec>(p_active);
            ...
            if (ok) {
                arma::vec tmp = arma::solve(arma::trimatl(L_prec.t()), mean_rhs, arma::solve_opts::fast);
                arma::vec m_beta = arma::solve(arma::trimatu(L_prec), tmp, arma::solve_opts::fast);

                arma::vec zz = arma::randn<arma::vec>(p_active);
                arma::vec noise = arma::solve(arma::trimatu(L_prec), zz, arma::solve_opts::fast);
        ...
        // alpha
        {
            ...
            double mean_alpha = var_alpha * (arma::accu(resid) + alpha0 / (h * sigmasq));
