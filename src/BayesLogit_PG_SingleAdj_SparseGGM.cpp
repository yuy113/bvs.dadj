...
Rcpp::List BayesLogit_PG_SingleAdj_SparseGGM(
  ...
  for (int iter = 0; iter < total_iter; ++iter) {
    ...
    if (block_size > 1) {
      // Block update: SW + Uncollapsed Gibbs (sparse X, single GGM CSC)
      // gamma is already std::vector<uint8_t>
      ...
      auto proposal = bvs_dadj_block::swendsen_wang_single(
          gamma, eta1, p, block_size, neigh_fn);
      ...
      for (int mh = 0; mh < n_mh_gamma; ++mh) {
        ...
        double ising = static_cast<double>(delta_g) * (mu + eta1 * neigh_dyn);
