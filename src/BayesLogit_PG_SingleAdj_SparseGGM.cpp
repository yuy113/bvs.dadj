...
//
// FIX: Reduced K=200 → K=20 (tail < 1e-8 for |z|<10).
//      C++ std::exponential_distribution avoids R RNG hook overhead.
...
// =============================================================================
// SECTION 4: SPARSE SINGLE-NETWORK PROPP-WILSON (1-eta)
//
// Kernel: eta1 * Z_active (dynamic GGM only).
...
// =============================================================================
// SECTION 5: SPARSE SINGLE-ETA MOLLER MH UPDATE
//
// Updates eta1 only. 2 Propp-Wilson calls.
// Sufficient stats: O(|E|) not O(p²).
// =============================================================================
static void moller_update_1eta_sparse(
  ...
  // Sufficient stats — edge products (single network)
...
// =============================================================================
// SECTION 6: PHASE TRANSITION DETECTION (1-eta, exported utility)
// =============================================================================
// [[Rcpp::export]]
Rcpp::IntegerMatrix
phase_transit_1eta_sparse(const Rcpp::IntegerVector &S_i,
                          const Rcpp::IntegerVector &S_p_csc, int p_ggm,
                          int T_max, double mu, double min_eta, double max_eta,
                          unsigned int num_rep, double step_size = 0.01) {
  std::vector<std::unordered_set<int>> Z_active(p_ggm);
  for (int col = 0; col < p_ggm; ++col) {
    for (int k = S_p_csc[col]; k < S_p_csc[col + 1]; ++k) {
      int row = S_i[k];
      Z_active[col].insert(row);
    }
  }
  ...
  std::vector<int> pw_x_up(p_ggm), pw_x_down(p_ggm), result(p_ggm);

  double eta_tmp = min_eta;
  for (unsigned int i = 0; i < len_eta; ++i) {
    for (unsigned int j = 0; j < num_rep; ++j) {
      proppwilson_sparse_1eta(Z_active, p_ggm, mu, eta_tmp, T_max, pw_x_up,
                              pw_x_down, result);
      int s = 0;
      for (int k = 0; k < p_ggm; ++k)
        s += result[k];
      gamma_output(i, j) = s;
    }
    eta_tmp += step_size;
...
//   PG augmented logistic regression with single-network Ising prior:
//     eta1 × sparse GGM adjacency (SSVS column sweep)
//
// Memory: O(n*p + p*d_max² + |E|)
// =============================================================================

// [[Rcpp::export]]
Rcpp::List BayesLogit_PG_SingleAdj_SparseGGM(
  ...
  // PW pre-allocated workspace (2 PW calls for single eta)
  ...
  for (int iter = 0; iter < total_iter; ++iter) {
    ...
    if (iter > 0 && iter % 5000 == 0) {
      ...
      int model_size = (int)active_idx.size(); // OPT: cached, no arma::accu
    ...
    //
    // FIX: PG K=20 + C++ RNG. Xb cached. No arma::find. No prec_sym copy.
    //      Progressive Cholesky jittering (5 attempts, 10×).
    // -----------------------------------------------------------------------
    ...
    // Intercept alpha (conjugate Gibbs) — uses cached Xb
    {
    ...
    // -----------------------------------------------------------------------
    // STEP C: Gamma via MH with single Ising prior (eta1 × GGM)
    // -----------------------------------------------------------------------
    ...
    for (int mh = 0; mh < n_mh_gamma; ++mh) {
      ...
      // Single Ising neighbourhood (GGM only)
    ...
    // Update Xb after gamma changes
    Xb = z;

    // -----------------------------------------------------------------------
    // STEP D: SigmaSq via log-normal MH
    //
    // OPT: Uses cached active_idx, no arma::find or arma::accu(gamma).
    // -----------------------------------------------------------------------
    {
    ...
    // -----------------------------------------------------------------------
    // STEP E: eta1 via Möller + sparse Propp-Wilson (single eta)
    // -----------------------------------------------------------------------
